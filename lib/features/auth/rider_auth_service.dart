...
class RiderAuthService {
  ...
  /// Primary login: driver code + 6-digit passcode via edge function.
  Future<RiderProfile> signInWithDriverPasscode({
    ...
    if (!RegExp(r'^\d{5}$').hasMatch(normalizedCode) ||
    ...
    try {
      response = await _client.functions.invoke(
        ...
        body: {'driver_code': normalizedCode, 'passcode': normalizedPasscode},
      );
...
final riderAuthServiceProvider = Provider<RiderAuthService>((ref) {
  return RiderAuthService(Supabase.instance.client);
});
...
final riderProfileProvider = FutureProvider<RiderProfile?>((ref) async {
  ...
  if (session == null) return null;
  try {
    return await ref.read(riderAuthServiceProvider).fetchProfile();
  } on RiderAuthFailure catch (e) {
    if (e == RiderAuthFailure.staffNotAllowed) return null;
    // fetchProfile is now defensive enough that any other RiderAuthFailure
    // means we couldn't even reach a fallback (e.g. session vanished).
    return null;
  } catch (_) {
    // Final safety net: keep the session, return null so UI can show a retry.
    return null;
  }
});
