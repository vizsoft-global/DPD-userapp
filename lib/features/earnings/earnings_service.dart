...
class EarningsService {
  ...
  /// Reads the same `driver_earnings_daily` table to compute lifetime totals
  /// — much cheaper than maintaining a dedicated RPC for this. Attendance is
  /// fetched from `driver_get_attendance(year, month)` rolled forward through
  /// the months we know about, but to keep this lightweight we just count
  /// distinct earn_date rows as a proxy for "working_days" and fall back to
  /// a 100% attendance label when we have nothing else.
  Future<PerformanceSummary> fetchPerformance() async {
    try {
      // Single fetch of just the small columns. For a typical driver we're
      // looking at a few hundred rows max — cheap enough not to bother with
      // server-side aggregation.
      final raw = await _client
          .from('driver_earnings_daily')
          .select('earn_date, deliveries');
      _networkStatus.recordRpcSuccess();
      var totalDeliveries = 0;
      final workingDayKeys = <String>{};
      for (final row in (raw as List).whereType<Map>()) {
        final deliveries = (row['deliveries'] as num?)?.toInt() ?? 0;
        totalDeliveries += deliveries;
        final day = row['earn_date']?.toString();
        if (day != null && day.isNotEmpty) workingDayKeys.add(day);
      }
