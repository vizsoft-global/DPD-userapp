...
class DutyTaskHandler extends TaskHandler {
  ...
  static const _inactivityLimit = Duration(minutes: 5);
  static const _mockLogCooldown = Duration(minutes: 2);
  DateTime? _lastSuccessfulReportAt;
  ...
  Future<void> onStart(DateTime timestamp, TaskStarter starter) async {
    ...
    _lastSuccessfulReportAt = timestamp;
  }
  ...
  Future<void> _tick(DateTime now, {bool force = false}) async {
    ...
    try {
      ...
      if (!enabled) {
        ...
        await _maybeAutoCheckout(token, now);
        return;
      }
  ...
  Future<void> _maybeAutoCheckout(String token, DateTime now) async {
    final lastOk = _lastSuccessfulReportAt;
    if (lastOk == null || now.difference(lastOk) < _inactivityLimit) {
      return;
    }
    ...
    try {
      ...
      final l10n = await _localizations();
      await _updateNotification(l10n.checkedOutInactive5Min);
      FlutterForegroundTask.sendDataToMain(
        jsonEncode({'event': 'auto_checkout_inactive'}),
      );
      ...
      final l10n = await _localizations();
      await _updateNotification(l10n.onDutyAutoCheckoutFailed);
    }
  }
